...
// Legacy bundled quick-start prompts removed from the product
const LEGACY_DEFAULT_PROMPT_TITLES = new Set([
  '📚 Explore AI Papers',
...
export function createFavoritesStorage(): FavoritePromptsStorage {
  return {
    ...
    getAllPrompts: async (): Promise<FavoritePrompt[]> => {
      ...
      const prompts = currentState.prompts.filter(prompt => !LEGACY_DEFAULT_PROMPT_TITLES.has(prompt.title));

      if (prompts.length !== currentState.prompts.length) {
